...
class RetinalCNN(nn.Module):
    def __init__(self, input_depth, input_height, input_width, output_size, hidden_size=128, device='None'):
        super().__init__()
        ...
        self.conv3d = nn.Conv3d(in_channels=1, out_channels=1, kernel_size=(input_depth, 1, 1), stride=1, padding=0).to(self.device)

        # 2D Convolutional layers
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=3, stride=1, padding=1).to(self.device)
    ...
    def _get_conv_output(self, input_depth, input_height, input_width):
        with torch.no_grad():
            ...
            dummy_input = dummy_input.view(1, 1, new_height, new_width)
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 1, new_height, new_width)
