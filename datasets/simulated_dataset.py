...
class MatrixDataset(Dataset):
    def __init__(self, target_matrix, length, combination_set=None, ratio_for_one=0.5,
                 initial_size=None):
        """
        Args:
    ...
    def generate_matrix(self, matrix_type):
        ...
        if matrix_type == 1:  # white noise with defined size
            ...
            initial_noise = torch.rand((1, *self.initial_size))
            ...
            return torch.bernoulli(p * torch.ones(self.dimensions))
        elif matrix_type == 3:  # random contrast full field
            base = torch.rand(self.dimensions[0])
            ...
            base = torch.rand(1, 1, self.dimensions[1], self.dimensions[2])
...
class MultiMatrixDataset(MatrixDataset):
    def __init__(self, target_matrices, length, combination_set=None, ratio_for_one=0.5,
        ...
        if isinstance(target_matrices, np.ndarray):
            target_matrices = torch.tensor(target_matrices, dtype=torch.float32)
        ...
        super().__init__(target_matrices[0], length, combination_set, ratio_for_one, initial_size)
    ...
    def __getitem__(self, idx):
        ...
        if self.add_noise:
            noise = torch.randn(1).item() * self.noise_level
            output_value += noise
