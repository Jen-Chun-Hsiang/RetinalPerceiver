...
def main():
    ...
    query_df = pd.DataFrame(query_array, columns=['experiment_id', 'neuron_id', 'stimulus_type_id'])
    query_array = pd.merge(query_df, experiment_info_table, on='experiment_id', how='left')
    query_array = query_array[['experiment_id', 'species_id', 'sex_id', 'neuron_id', 'stimulus_type_id']]
    query_array['neuron_unique_id'] = query_array['experiment_id'] * 10000 + query_array['neuron_id']
    query_array = query_array[['experiment_id', 'species_id', 'sex_id', 'neuron_unique_id', 'stimulus_type_id']]
