    ...
    def __getitem__(self, idx):
        ...
        if self.is_in_bulk:
            try:
                ...
                images_3d = images_3d.unsqueeze(0).repeat(num_cells, 1, 1, 1, 1).to(self.device)  # Adding an extra dimension to simulate batch size
            except MemoryError:
                print(f'number of cell: {num_cells}')
        else:
            images_3d = images_3d.unsqueeze(0).to(self.device)
