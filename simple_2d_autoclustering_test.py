...
# from some_module import get_2d_sincos_positional_encoding
...
for epoch in range(num_epochs):
    ...
    print(f"Epoch {epoch+1}/{num_epochs}, Total Loss: {epoch_total_loss:.6f}, "
...
model_save_path = os.path.join(folder_path, f"{exp_name}_model_final.pth")
...
print(f"Loss results saved at {loss_save_path_json}")
...
loss_save_path_npz = os.path.join(folder_path, f"{exp_name}_losses.npz")
loss_data = np.load(loss_save_path_npz)

# Retrieve the losses stored during training.
epochs = loss_data["epochs"]
total_loss = loss_data["total_loss"]
reg_loss = loss_data["reg_loss"]
cluster_loss = loss_data["cluster_loss"]
...
for i, cell in enumerate(dataset.cell_properties):
    ...
    # print(f"i: {i}")
    # print(f"is_center_known: {is_center_known}")
    if is_center_known is False:
      unknown_center_id = i - dataset.A
      # print(f"unknown_center_id: {unknown_center_id}")
